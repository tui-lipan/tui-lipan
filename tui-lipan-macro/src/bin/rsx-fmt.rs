...
use rsx_format::format_file_contents;

fn main() {
    if let Err(err) = run() {
        eprintln!("rsx-fmt: {err}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if check && changed {
        return Err("some rsx! macros need formatting".into());
    }

    // Also run `cargo fmt --all` (or `--check`) on the regular Rust code.
    if !stdin {
        ...
        if !status.success() {
            return Err("cargo fmt --all failed".into());
        }
    }
...
fn print_help() {
    println!(
        "Usage: rsx-fmt [--check] [files...]\n       rsx-fmt --stdin\n\nFormats rsx! macro bodies inside Rust source files. When no files are provided, all Rust files under the current directory are formatted."
    );
}
