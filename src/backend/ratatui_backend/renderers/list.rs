...
pub(crate) fn render_list(params: ListRenderParams<'_, '_, '_>) {
    ...
    if len == 0 {
        if let Some(text) = empty_text {
            ...
            let pad_left = item_horizontal_padding
                .left
                .saturating_add(empty_text_padding.left);
            let pad_right = item_horizontal_padding
                .right
                .saturating_add(empty_text_padding.right);
            let empty_inner = Rect {
                x: inner.x.saturating_add(pad_left as i16),
                y: inner.y.saturating_add(empty_text_padding.top as i16),
                w: inner.w.saturating_sub(pad_left.saturating_add(pad_right)),
                h: inner.h.saturating_sub(empty_text_padding.vertical()),
            };
            if empty_inner.w == 0 || empty_inner.h == 0 {
                return;
            }
...
pub(crate) fn render_list_node(
    ...
    render_list(ListRenderParams {
        ...
        empty_text_padding: list_node.empty_text_padding,
        is_focused,
        is_hovered,
