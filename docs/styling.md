...
neighbor's border-title text (or spaces that sit next to that text) when the seam already carries
that neighbor's box-drawing. Ordinary underlay content is still replaced. Plain backdrop spaces
still accept a border. Use `Replace` when an occluding frame should wipe what is underneath.
