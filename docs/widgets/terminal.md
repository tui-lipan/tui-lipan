...
`extend_link_hover_style`, and `inherit_link_hover_style` builders customize that feedback. Modifier
state arrives on pointer events, so pressing Ctrl over an already stationary link applies the style
on the next pointer movement rather than on the press itself.
...
- `Ctrl+C` with a non-empty selection in the focused terminal copies to the clipboard instead of running an app shortcut on the same key. A selection in some other widget or terminal does not: the focused terminal's child still receives `Ctrl+C`.
