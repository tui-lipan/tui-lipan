    ...
    fn init(&mut self, ctx: &mut Context<Self>) -> Option<Command> {
        Some(ctx.link().command(|link| {
            ...
            link.send(Msg::Seeded(vec![
                TodoItem {
                    id: 1,
                    text: "Dogfood rsx! with loops".to_string(),
                    done: false,
                },
                TodoItem {
                    id: 2,
    ...
    fn view(&self, ctx: &Context<Self>) -> Element {
                ...
                HStack {
                    ...
                    Frame {
                        ...
                        VStack {
                            ...
                            Text {
                                content: "• Active panel border follows focus",
                                style: Style::new().fg(Color::indexed(240)),
                            },
                            Text {
                                content: "• Delete shows a modal dialog",
                                style: Style::new().fg(Color::indexed(240)),
                            },
                            Text {
                                content: "• Toasts appear on actions",
                                style: Style::new().fg(Color::indexed(240)),
                            },
                        },
                    },
        ...
        if let Some(delete_id) = confirm_id {
            let modal = Modal::new()
                            ...
                            HStack::new()
                                ...
                                .child(
                                    Button::outlined("Cancel")
                                        ...
                                        .on_key(ctx.link().key_handler(
                                            |k: KeyEvent| match k.code {
                                ...
                                .child(
                                    Button::outlined("Delete")
                                        ...
                                        .on_click(
                                            ctx.link()
                                                .callback(move |_| Msg::ConfirmDelete(delete_id)),
                                        )
                                        .on_key(ctx.link().key_handler(move |k: KeyEvent| {
                                            match k.code {
                                                KeyCode::Enter => {
                ...
                // .backdrop_style(Style::new().bg(Color::indexed(232)))
                // .frame_style(Style::new().bg(palette::RED))
            ...
            ZStack::new().child(main_content).child(modal).into()
        } else {
            main_content
        }
    }
