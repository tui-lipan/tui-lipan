...
per-PR note fragments. The `/changelog` command defines release-note structure.
Rosie writes the prose from each PR Summary in the tagged range, so keep PR
titles, summaries, migration steps, and user documentation concrete.
...
Trusted Publishing. The same frozen notes become the GitHub Release body.
