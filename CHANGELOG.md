...
- `DraggableTabBar::caps(Option<(char, char)>)` draws `(left, right)` end-cap glyphs around the
  ...
  not drop caps. On `FrameLine`, caps replace the padding cells around the body and the accent
  marker stays. Defaults to `None` (flat padding). See `docs/widgets/tabs.md`.
